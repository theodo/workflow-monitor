...
export class PubSubAsyncIterator<T> implements AsyncIterator<T> {
  ...
  private subscriberOptions: { [key: string]: any };
  ...
  private async pushValue(event: T) {
    ...
    if (this.pullQueue.length !== 0) {
      this.pullQueue.shift()(
        this.running ? { value: event, done: false } : { value: undefined, done: true },
      );
    } else {
  ...
  private subscribeAll() {
    return Promise.all(
      this.eventsArray.map(eventName =>
        this.pubsub.subscribe(eventName, this.pushValue.bind(this), this.subscriberOptions),
      ),
    );
  }
