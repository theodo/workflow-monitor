...
import { GQLServerAllEvents, GQLOperationResult, SERVER_EVENT_TYPES } from '../protocol';
...
class Client {
  ...
  public onError = (listener: EventEmitter.ListenerFn): Function => this.on('error', listener);

  public onMessage = (listener: EventEmitter.ListenerFn): Function => this.on('message', listener);
    ...
    try {
      ...
      switch (message.type) {
        case SERVER_EVENT_TYPES.GQL_OP_RESULT: {
          this.operationProcessor.processOperationResult(message as GQLOperationResult);
          break;
        }
