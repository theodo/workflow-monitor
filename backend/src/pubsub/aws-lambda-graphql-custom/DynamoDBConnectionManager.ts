...
import { IConnection, IConnectEvent, IConnectionManager, ISubscriptionManager } from './types';
...
class DynamoDBConnectionManager implements IConnectionManager {
  ...
  sendToConnection = async (connection: IConnection, payload: string | Buffer): Promise<void> => {
    ...
    try {
      await managementApi.postToConnection({ ConnectionId: id, Data: payload }).promise();
    } catch (e) {
